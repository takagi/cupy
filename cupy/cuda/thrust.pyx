...
cpdef argsort(dtype, size_t idx_start, size_t data_start,
    ...
    # TODO(takagi): Support float16 and bool
    if dtype == numpy.int8:
        _argsort[common.cpy_byte](_idx_start, _data_start, shape)
    elif dtype == numpy.uint8:
        _argsort[common.cpy_ubyte](_idx_start, _data_start, shape)
    elif dtype == numpy.int16:
        _argsort[common.cpy_short](_idx_start, _data_start, shape)
    elif dtype == numpy.uint16:
        _argsort[common.cpy_ushort](_idx_start, _data_start, shape)
    elif dtype == numpy.int32:
        _argsort[common.cpy_int](_idx_start, _data_start, shape)
    elif dtype == numpy.uint32:
        _argsort[common.cpy_uint](_idx_start, _data_start, shape)
    elif dtype == numpy.int64:
        _argsort[common.cpy_long](_idx_start, _data_start, shape)
    elif dtype == numpy.uint64:
        _argsort[common.cpy_ulong](_idx_start, _data_start, shape)
    elif dtype == numpy.float32:
        _argsort[common.cpy_float](_idx_start, _data_start, shape)
    elif dtype == numpy.float64:
        _argsort[common.cpy_double](_idx_start, _data_start, shape)
    else:
        raise NotImplementedError('Sorting arrays with dtype \'{}\' is not '
                                  'supported'.format(dtype))
